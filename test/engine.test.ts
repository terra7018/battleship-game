...
describe("placement", () => {
  ...
  it("randomFleetRemaining fills by identity when ships were placed out of fleet order", () => {
    ...
    expect(b.ships.map((s) => s.name).sort()).toEqual(
      FLEET.map((s) => s.name).sort(),
    );
  });
...
describe("AI difficulty", () => {
  ...
  it.each(LEVELS)(
    "%s sinks a random fleet in <= 100 shots without repeating a cell",
    (level) => {
      for (let seed = 1; seed <= 40; seed++) {
        const rng = seeded(seed * 31 + 7);
        expect(playOut(createAi(level, rng), rng)).toBeLessThanOrEqual(
          SIZE * SIZE,
        );
      }
    },
  ...
  it("Hard restricts to placements covering pending hits", () => {
    ...
    ai.notify(b, idx(5, 4), fireAt(b, idx(5, 4)).result);
    expect([idx(5, 3), idx(5, 5), idx(4, 4), idx(6, 4)]).toContain(
      ai.chooseTarget(b),
    );
...
describe("Game flow", () => {
  ...
  it("finds the latest shot per side", () => {
    const ev = (by: ShotEvent["by"], cell: number): ShotEvent => ({
      by,
      cell,
      result: "miss",
      ship: null,
...
describe("stats", () => {
  ...
  it("reports zero accuracy with no shots", () => {
    ...
    expect(computeStats([], createBoard()).player).toEqual({
      shots: 0,
      hits: 0,
      accuracy: 0,
    });
  ...
  it("round-trips through JSON and rejects malformed data", () => {
    ...
    expect(
      parseRecord(
        '{"wins":"3","losses":0,"streak":0,"bestStreak":0,"bestWinShots":null}',
      ),
    ).toEqual(EMPTY_RECORD);
  ...
  it("formats the record and stats lines", () => {
    ...
    expect(formatRecord(r)).toBe(
      "Record: 3W–2L · Streak 2 · Best streak 2 · Best win 41 shots",
    );
    const s = computeStats(
      [ev("player", "hit"), ev("ai", "miss")],
      createBoard(),
    );
