...
const sinkDurationMs = (len: number): number =>
  SINK_CELL_MS + SINK_STAGGER_MS * (len - 1);
...
function commitRecord(): PlayerRecord {
  ...
  const rec = pending.reduce(
    (r, p) => updateRecord(r, p.winner, p.shots),
    base,
  );
  try {
...
const label = (i: number): string =>
  `${String.fromCharCode(65 + rowOf(i))}${colOf(i) + 1}`;

function previewCells(): { cells: number[]; ok: boolean } | null {
  if (hoverCell === null || game.phase !== "placement" || game.fleetComplete)
    return null;
...
function renderBoard(
  ...
  const lastShot = lastShotBy(
    game.log,
    el === enemyBoardEl ? "player" : "ai",
  )?.cell;
  for (let i = 0; i < cells.length; i++) {
    ...
    if (revealShips && shipId !== -1 && !shot && shipId !== lifted)
...
function renderFleet(
  ...
  FLEET.forEach((_, k) => {
    ...
    if (ship && active && ship.cells.some((c) => active.has(c)))
      li.classList.add("sinking");
    if (showPending && !ship && k === board.ships.length)
...
function render(): void {
  ...
  turnEl.textContent =
    game.phase === "player-turn" ? "Your turn" : "Enemy turn";
...
function statusText(): string {
  switch (game.phase) {
    ...
    case "game-over": {
      ...
      return inspecting
        ? `${outcome} ${formatStats(computeStats(game.log, game.player))}`
        : outcome;
    }
  }
}
...
playerBoardEl.addEventListener("mouseleave", () => {
  const byKeyboard =
    !pointerInput && playerBoardEl.contains(document.activeElement);
...
function endDrag(commit: boolean, pointerEnded: boolean): void {
  ...
  if (playerBoardEl.hasPointerCapture(pointerId))
    playerBoardEl.releasePointerCapture(pointerId);
  ...
  (e) => {
    ...
    const id =
      e instanceof PointerEvent
        ? e.pointerId
        : spentPointers.values().next().value;
...
function placeAt(i: number): void {
  ...
  const cells = shipCells(
    rowOf(i),
    colOf(i),
    game.nextShip.length,
    orientation,
  );
  if (!canPlace(game.player, cells)) return;
  placeShip(game.player, game.nextShip, cells!);
  render();
  if (game.fleetComplete && playerBoardEl.contains(document.activeElement))
    startBtn.focus();
}
...
function renderStats(s: GameStats): void {
  const side = (x: SideStats): string =>
    `${x.shots} shots · ${x.hits} hits · ${x.accuracy}%`;
...
randomBtn.addEventListener("click", () => {
  endDrag(false, false);
  while (game.player.ships.length)
...
difficultySelect.addEventListener("change", () => {
  difficulty = isDifficulty(difficultySelect.value)
